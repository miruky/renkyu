...
- 「iCal保存」で表示中の年の祝日を `.ics` として書き出し、カレンダーアプリに取り込める
- ヘッダーのアイコンで配色を「端末の設定に合わせる・ライト・ダーク」と切り替えられる(描画前に確定するためちらつかない)
...
- `src/lib/` — 依存ゼロのロジック
  ...
  - `ics.ts` — 祝日のiCalendar(.ics)書き出し
  - `theme.ts` — 配色テーマ選好の解決・保存
